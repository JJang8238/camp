...
public class CampDAO {
    ...
    // ══════════════════════════════════════════════════════════════════
    // ✅ 2단계: 조합 보너스 (여러 태그 동시 선택 시 추가 점수)
    // ══════════════════════════════════════════════════════════════════
    ...
    // ══════════════════════════════════════════════════════════════════
    // ✅ 3단계: 연관 태그 자동 확장 (선택한 태그의 연관 태그도 점수 반영)
    // ══════════════════════════════════════════════════════════════════
    ...
    static {
        ...
        RELATED_TAGS.put("반려견",     Arrays.asList("애견동반", "펫")); // ✅ "반려동물" 제거 → 반려동물불가 오매칭 방지
    ...
    // ══════════════════════════════════════════════════════════════════
    // ✅ 캠핑 스타일 → 태그 매핑 (메인 AI 질문 결과를 태그로 변환)
    // ══════════════════════════════════════════════════════════════════
    ...
    private static synchronized void detectReservationColumns() {
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            while (cols.next()) columnNames.add(cols.getString("COLUMN_NAME").toLowerCase());
            ...
            for (String c : inC)  { if (columnNames.contains(c)) { COL_CHECK_IN  = c; break; } }
            for (String c : outC) { if (columnNames.contains(c)) { COL_CHECK_OUT = c; break; } }
    ...
    private void buildEnhancedScoreClause(
            StringBuilder sql, List<String> scoreParams,
            String keyword, String type, String loc, String facility, String styles) {
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            ...
            scoreParams.add(kw); scoreParams.add(kw); scoreParams.add(kw);
        }
        ...
        if (type != null && !type.trim().isEmpty())
            for (String t : type.split(",")) if (!t.trim().isEmpty()) allTags.add(t.trim());
        if (facility != null && !facility.trim().isEmpty())
            for (String f : facility.split(",")) if (!f.trim().isEmpty()) allTags.add(f.trim());

        // ✅ 태그별 가중치 점수
        for (String tag : allTags) {
            int weight = TAG_WEIGHTS.getOrDefault(tag, 20);
            sql.append("+ CASE WHEN type LIKE ? OR tags LIKE ? THEN " + weight + " ELSE 0 END ");
            scoreParams.add("%" + tag + "%"); scoreParams.add("%" + tag + "%");
        ...
        for (Map.Entry<String, Integer> combo : COMBO_BONUS.entrySet()) {
            ...
            if (has0 && has1) {
                sql.append("+ CASE WHEN (type LIKE ? OR tags LIKE ?) AND (type LIKE ? OR tags LIKE ?) THEN " + combo.getValue() + " ELSE 0 END ");
                scoreParams.add("%" + ct[0] + "%"); scoreParams.add("%" + ct[0] + "%");
                scoreParams.add("%" + ct[1] + "%"); scoreParams.add("%" + ct[1] + "%");
            }
        }
        ...
        if (loc != null && !loc.trim().isEmpty()) {
            for (String l : loc.split(",")) {
                ...
                for (String expanded : expandLocationKeyword(l.trim())) {
                    ...
                    scoreParams.add("%" + expanded + "%"); scoreParams.add("%" + expanded + "%");
                }
            }
        }
    ...
    // ══════════════════════════════════════════════════════════════════
    //  캠핑장 목록 페이징 (styles 파라미터 없는 기존 호환 버전)
    // ══════════════════════════════════════════════════════════════════
    public List<Product> getCampListPaging(
            String keyword, String type, String loc, String facility,
            String sort, String checkIn, String checkOut, int page, int pageSize) {
        return getCampListPagingWithStyles(keyword, type, loc, facility, sort, checkIn, checkOut, null, page, pageSize);
    }

    // ══════════════════════════════════════════════════════════════════
    //  캠핑장 목록 페이징 (styles 포함 버전)
    // ══════════════════════════════════════════════════════════════════
    public List<Product> getCampListPagingWithStyles(
            String keyword, String type, String loc, String facility,
            String sort, String checkIn, String checkOut, String styles,
        ...
        if (useDate) {
            ...
            if (df != null) { sql.append(df); whereParams.add(checkIn); whereParams.add(checkOut); }
        }

        appendWhereFilters(sql, whereParams, keyword, type, loc, facility, styles);

        if ("priceAsc".equals(sort))       sql.append("ORDER BY price ASC, id DESC ");
        else if ("priceDesc".equals(sort)) sql.append("ORDER BY price DESC, id DESC ");
        else                               sql.append("ORDER BY recommend_score DESC, id DESC ");
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            while (rs.next()) {
                ...
                p.setAddress(rs.getString("address")); p.setType(rs.getString("type"));
                p.setTags(rs.getString("tags")); p.setPrice(rs.getInt("price"));
    ...
    public int getCampCount(String keyword, String type, String loc, String facility,
        ...
        if (useDate) {
            String df = buildDateFilterSql();
            if (df != null) { sql.append(df); params.add(checkIn); params.add(checkOut); }
        }
    ...
    //  공통 WHERE 필터 추가
    // ══════════════════════════════════════════════════════════════════
    private void appendWhereFilters(StringBuilder sql, List<String> params,
            String keyword, String type, String loc, String facility) {
    ...
    private void appendWhereFilters(StringBuilder sql, List<String> params,
        ...
        if (type != null && !type.trim().isEmpty()) {
            ...
            for (String t : types) {
                ...
                params.add("%" + t.trim() + "%"); params.add("%" + t.trim() + "%");
                first = false;
            }
        ...
        if (loc != null && !loc.trim().isEmpty()) {
            ...
            for (String l : loc.split(",")) {
                if (!l.trim().isEmpty()) exp.addAll(expandLocationKeyword(l.trim()));
            }
            sql.append("AND (");
            for (int i = 0; i < exp.size(); i++) {
                if (i > 0) sql.append(" OR ");
                sql.append("address LIKE ? OR tags LIKE ?");
                params.add("%" + exp.get(i) + "%"); params.add("%" + exp.get(i) + "%");
            }
            sql.append(") ");
        }
        if (facility != null && !facility.trim().isEmpty()) {
        ...
        // ✅ 반려동물 스타일 선택 시 반려동물불가 캠핑장 명시적 제외
        if (styles != null) {
    ...
    private List<String> expandLocationKeyword(String loc) {
        ...
        switch (loc.trim()) {
            case "경상도":  list.addAll(Arrays.asList("경상도","경상북도","경상남도","경북","경남")); break;
            case "전라도":  list.addAll(Arrays.asList("전라도","전라북도","전라남도","전북","전남")); break;
            case "충청도":  list.addAll(Arrays.asList("충청도","충청북도","충청남도","충북","충남")); break;
            case "강원도":  list.addAll(Arrays.asList("강원도","강원특별자치도")); break;
            case "제주":    list.addAll(Arrays.asList("제주","제주도","제주특별자치도")); break;
            case "서울/경기":          list.addAll(Arrays.asList("서울","서울특별시","경기","경기도")); break;
            case "인천/강화도":        list.addAll(Arrays.asList("인천","인천광역시","강화","강화도")); break;
            case "거제/남해/통영":     list.addAll(Arrays.asList("거제","남해","통영")); break;
            case "안면도/태안":        list.addAll(Arrays.asList("안면도","태안")); break;
            case "대부도/선재도/영흥도": list.addAll(Arrays.asList("대부도","선재도","영흥도")); break;
            case "춘천/홍천":          list.addAll(Arrays.asList("춘천","홍천")); break;
            default: list.add(loc.trim()); break;
        }
        return list;
    }
    ...
    public Camp getCampById(int id) {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            if (rs.next()) {
                camp = new Camp();
                camp.setId(rs.getInt("id")); camp.setName(rs.getString("name"));
                camp.setAddress(rs.getString("address")); camp.setType(rs.getString("type"));
                camp.setTags(rs.getString("tags")); camp.setPrice(rs.getInt("price"));
                camp.setImage(rs.getString("image")); camp.setStatus(rs.getString("status"));
    ...
    public boolean insertCampFromCsv(Camp camp) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO camps (name,address,type,tags,price,image,description,status) VALUES (?,?,?,?,?,?,?,?)")) {
            ps.setString(1,camp.getName()); ps.setString(2,camp.getAddress());
            ps.setString(3,camp.getType()); ps.setString(4,camp.getTags());
            ps.setInt(5,camp.getPrice());   ps.setString(6,camp.getImage());
            ps.setString(7,camp.getDescription()); ps.setString(8,"active");
    ...
    public static boolean insertCamp(Camp camp, int ownerId) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO camps (name,address,type,tags,price,image,description,status,owner_id) VALUES (?,?,?,?,?,?,?,?,?)")) {
            ps.setString(1,camp.getName()); ps.setString(2,camp.getAddress());
            ps.setString(3,camp.getType()); ps.setString(4,camp.getTags());
            ps.setInt(5,camp.getPrice());   ps.setString(6,camp.getImage());
            ps.setString(7,camp.getDescription()); ps.setString(8,camp.getStatus());
            ps.setInt(9,ownerId);
            return ps.executeUpdate() > 0;
        } catch (Exception e) { e.printStackTrace(); }
        return false;
    ...
    public static List<Camp> getCampsByOwnerId(int ownerId) {
        List<Camp> list = new ArrayList<>();
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM camps WHERE owner_id = ? ORDER BY id DESC")) {
            ps.setInt(1, ownerId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Camp camp = new Camp();
    ...
    public static boolean deleteCamp(int campId, int ownerId) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM camps WHERE id = ? AND owner_id = ?")) {
            ps.setInt(1, campId); ps.setInt(2, ownerId);
            return ps.executeUpdate() > 0;
        } catch (Exception e) { e.printStackTrace(); }
        return false;
    }
    ...
    public static boolean updateCampStatus(int campId, int ownerId, String newStatus) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement("UPDATE camps SET status = ? WHERE id = ? AND owner_id = ?")) {
            ps.setString(1, newStatus); ps.setInt(2, campId); ps.setInt(3, ownerId);
            return ps.executeUpdate() > 0;
        } catch (Exception e) { e.printStackTrace(); }
        return false;
    }
    ...
    public static boolean updateCamp(int campId, int ownerId, String name, String address,
            String type, String tags, int price, String description, String status) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                "UPDATE camps SET name=?,address=?,type=?,tags=?,price=?,description=?,status=? WHERE id=? AND owner_id=?")) {
            ps.setString(1,name); ps.setString(2,address); ps.setString(3,type);
            ps.setString(4,tags); ps.setInt(5,price); ps.setString(6,description);
            ps.setString(7,status); ps.setInt(8,campId); ps.setInt(9,ownerId);
            return ps.executeUpdate() > 0;
        } catch (Exception e) { e.printStackTrace(); }
        return false;
    }
